...
try {
    if ($action === 'select') {
        ...
        } else if ($table === 'solicitudes_compras' && strpos($select, 'usuarios') !== false) {
             foreach ($result as &$row) {
                $stmtU = $db->prepare("SELECT nombre_completo FROM usuarios WHERE id_usuario = ?");
                ...
                if (strpos($select, 'detalle_solicitud_compra') !== false) {
                    $stmtD = $db->prepare("SELECT d.*, i.nombre, i.unidad_medida FROM detalle_solicitud_compra d LEFT JOIN insumos i ON d.id_insumo = i.id_insumo WHERE d.id_solicitud_compra = ?");
                    $stmtD->execute([$row['id_solicitud_compra']]);
                    $detalles = $stmtD->fetchAll(PDO::FETCH_ASSOC);
                    foreach($detalles as &$det) {
                        $det['insumos'] = ['nombre' => $det['nombre'], 'unidad_medida' => $det['unidad_medida']];
                    }
                    $row['detalle_solicitud_compra'] = $detalles;
                }
             }
        ...
        foreach($items as $item) {
            ...
            if ($id) {
                ...
                $pkMap = [
                    ...
                    'compras_ordenes' => 'id_orden',
                    'compras_detalle' => 'id_detalle',
                    'compras_entradas' => 'id_entrada',
                    'caja_menor_movimientos' => 'id_movimiento',
                    'ventas_cotizaciones' => 'id_cotizacion',
                    'ventas_detalle' => 'id_detalle',
                    'ventas_perdidas' => 'id_perdida',
                    'kardex_movimientos' => 'id_kardex',
                    'cierres_mensuales' => 'id_cierre',
                    'fayb06_recibidos' => 'id_recibido',
                    'solicitudes_compras' => 'id_solicitud_compra',
                    'detalle_solicitud_compra' => 'id_detalle_compra'
                ];
